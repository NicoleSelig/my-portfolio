  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='flex w-full flex-col items-start space-y-6 mt-12 text-xl text-center font-medium leading-relaxed font-in'>
      <input
        ...
        {...register("entries", { required: 'Name is required', maxLength: {value: 180, message: 'Name must be a max of 100 characters' }}) }
      />
      ...
      <input
        ...
        placeholder="Email"
        className="w-2/5 rounded"
        {...register("entries", { required: 'Email is required', pattern: { value: /^\S+@\S+$/i, message: 'Valid email required' }})}
      />
      <ContactError errors={errors} name="email" />
      <textarea placeholder="What would you like to chat about?" className="w-full rounded" {...register("entries", { required: "A message is required.", })} />
